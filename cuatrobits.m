...
a=4;
for i=1:1:length(L)
    if k<=length(L) && l<=length(L) && p<=length(L) && a<=length(L)
        y1(k:k+3)=[L(k) L(k) L(k) L(k)];
        y2(k:k+3)=[L(l) L(l) L(l) L(l)];
        y3(k:k+3)=[L(p) L(p) L(p) L(p)];
        y4(k:k+3)=[L(a) L(a) L(a) L(a)];
        k=k+4;
        l=l+4;
        p=p+4;
        a=a+4;
...
k=1;
for i=1:4:length(sal1)
    x1(k:k+3)=[sal1(i) sal2(i) sal3(i) sal4(i)];
    k=k+4;
end
...
%
L1=x1(5:length(x1)); %concatenación del vector reconstruido y 0 0 0 para que el tamaño del vector sea múltiplo de 8 bits
...
figure(2)
image(recon);
title('Imagen Recuperada - 16QAM')

%%
figure(3)
plot(No2,errores);
title('Errores vs Potencia del ruido - 16QAM')
