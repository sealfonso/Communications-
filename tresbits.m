...
p=3;
for I=1:1:length(L)
    if k<=length(L) && l<=length(L) && p<=length(L)
        y1(k:k+2)=[L(k) L(k) L(k)];
        y2(k:k+2)=[L(l) L(l) L(l)];
        y3(k:k+2)=[L(p) L(p) L(p)];
        k=k+3;
        l=l+3;
        p=p+3;
...
k=1;
for i=1:3:length(sal1)
    x1(k:k+2)=[sal1(i) sal2(i) sal3(i)];
    k=k+2;
end
...
L1=x1(2:length(x1)); %concatenación del vector reconstruido y 0 0 0 para que el tamaño del vector sea múltiplo de 8 bits
...
figure(2)
image(recon);
title('Imagen Recuperada - 8PSK')

%%
figure(3)
plot(No2,errores);
title('Errores vs Potencia del ruido - 8PSK')
